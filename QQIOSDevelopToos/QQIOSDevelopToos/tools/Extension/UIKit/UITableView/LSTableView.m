...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (touch.tapCount == 1) {
        ...
        if (self.touchDelegate && [self.touchDelegate respondsToSelector:@selector(tableViewWillTouch:touchPoint:)]) {
            [self.touchDelegate tableViewWillTouch:self touchPoint:self.touchPoint];
        }
    }
}
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (!self.dragging) {
        if (touch.tapCount == 1) {
            ...
            {// 限定像素内的小偏移认为是点击
                [self performSelector:@selector(touchTableView) withObject:nil afterDelay:.2];
            }
        }
    }
}
...
- (void)touchTableView
{
    if (self.touchDelegate && [self.touchDelegate respondsToSelector:@selector(tableViewTouchUpInside:)]) {
        [self.touchDelegate tableViewTouchUpInside:self];
    }
}
